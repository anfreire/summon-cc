...
# from "hung" without capturing the pane. When the answered tool is AskUserQuestion, the
# pending question.json is archived — its absence means "no question pending".
...
if [ "$(printf '%s' "$p" | jq -r '.tool_name // ""' 2>/dev/null)" = "AskUserQuestion" ] \
   && [ -f "$d/question.json" ]; then
  mv -f "$d/question.json" "$d/question.answered.json" 2>/dev/null
fi
exit 0
